<template>
  <main>
    ...
    <!--<v-text-field label="Chain:" v-model="chain['Chain Name']" disabled v-if="type === 'edit'"></v-text-field>-->
    <!--<v-text-field label="Chain:" v-model="chain['Chain Name']" v-else></v-text-field>-->
    <v-select
      v-if="chainType == 'edit'"
    ...
    mounted() {
      ...
      providerServices.getChainList({}).then(function (res) {
        ...
        if (that.chainType !== 'edit') {
          that.chain_id = res.chain_list[0]['Chain ID']
        } else {
          that.chain_id = that.chain['Chain ID']
        }
      })
      ...
      planServices.getContractList({}).then(function (res) {
        ...
        if (that.chainType !== 'edit') {
          ...
          that.contract_id = that.chain['contract_id']
        }
      })
    },
    methods: {
