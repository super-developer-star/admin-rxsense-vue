        ...
        <template v-if="active === 'main'">
          ...
          <div class="my-scroll-enabled">
            <v-layout row wrap>
              ...
              <v-flex xs12>
                <v-card-text class="py-0">
                  <v-text-field label="Supply Limit" v-model="info['*Supply Limit']" type="Number"
                                suffix="% of Days Supply" :rules="[rules.supplyRules]" hint="'Supply Limit' is less than 99999.99!"></v-text-field>
                </v-card-text>
              </v-flex>
              <v-flex xs3>
                <v-card-text class="py-0">
                  <v-text-field label="Min Days:" v-model="info['*Min Days']" type="Number"
                                :rules="[rules.dayRules, rules.ldTMax]" hint="'Min Days' is less than 9999 !"></v-text-field>
                </v-card-text>
              </v-flex>
              <v-flex xs3>
                <v-card-text class="py-0">
                  <v-text-field label="Max Days:" v-model="info['*Max Days']" type="Number"
                                :rules="[rules.dayRules, rules.gdTMin]" hint="'Max Days' is less than 9999 !"></v-text-field>
                </v-card-text>
              </v-flex>
              <v-flex xs3>
                <v-card-text class="py-0">
                  <v-text-field label="Min Quantity:" v-model="info['*Min Qty']" type="Number"
                                :rules="[rules.qtyRules, rules.lqTMax]" hint="'Min Qty' is less than 99999.99 !"></v-text-field>
                </v-card-text>
              </v-flex>
              <v-flex xs3>
                <v-card-text class="py-0">
                  <v-text-field label="Max Quantity:" v-model="info['*Max Qty']" type="Number"
                                :rules="[rules.qtyRules, rules.gqTMin]" hint="'Max Qty' is less than 99999.99 !"></v-text-field>
                </v-card-text>
              </v-flex>
              <v-flex xs3>
                <v-card-text class="py-0">
              ...
              <v-flex xs3>
                <v-card-text class="py-0">
                  <date-picker label="Picker in menu" v-model="info['*Start Dt']"></date-picker>
                </v-card-text>
              </v-flex>
              <v-flex xs3>
                <v-card-text>
...
//      Open Member Drug Edit Form signal
      ...
      window.Vue.$on('drugSelected', function (item) {
        if (item.NDC) {
          ...
          that.info['Drug Name'] = item['Product Type Desc'] + ' ' + item['Form Desc'] + ' ' + item['Strength']
        } else {
    ...
    methods: {
      setValues() {
        ...
        if (this.info['*Supply Limit'] === null) this.info['*Supply Limit'] = '0'
        ...
        this.info = Object.assign({}, {...this.info}, {
          '*Max Qty': (this.info['*Max Qty'] || 0).toFixed(3),
          '*Min Qty': (this.info['*Min Qty'] || 0).toFixed(3),
          '*Supply Limit': parseFloat(this.info['*Supply Limit']).toFixed(3)
        })
        ...
        formularyServices.getTermCodeList({}).then(function (res) {
          ...
          that.info.qty_term_cd = that.info.qty_term_cd.trim() || '-1'
        })
      },
      open: function () {
        this.show = true
